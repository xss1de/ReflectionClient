...
public class InfoButton extends Component {
	...
	public InfoButton(Button button, Module mod, int offset) {
		this.parent = button;
		this.mod = mod;
	...
	public void renderComponent() {
		...
		Minecraft.getMinecraft().fontRenderer.drawStringWithShadow("Info", (parent.parent.getX() + 7) * 2, (parent.parent.getY() + offset + 2) * 2 + 5, -1);
		GL11.glPopMatrix();
	}
	...
	public void mouseClicked(int mouseX, int mouseY, int button) {
		if(isMouseOnButton(mouseX, mouseY) && button == 0 && this.parent.open) {
			ChatUtil.sendChatMessage("§o"+this.mod.getDescription()+".");
		}
	}
